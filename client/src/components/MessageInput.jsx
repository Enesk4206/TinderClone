...
const MessageInput = ({match}) => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) =>{
            if(emojiPickerRef.current && !emojiPickerRef.current.content(event.target)){
    ...
    return (
        <form onSubmit={handleSendMessage} className='flex relative'>
            <button 
                ...
                <button type='submit' className='bg-pink-500 text-white p-3 rounded-r-lg hover:bg-pink-500 transition-colors focus:outline-none focus:ring-2 focus:ring-pink-300'>
                    <Send size={24}/>
                </button>
                    {showEmojiPicker && (
